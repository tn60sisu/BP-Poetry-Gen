...
def sen_confusion_matrix(pred, correct):
  sen = {
    'Annoyance': 0,
    'Awe/Sublime': 1,
    'Beauty/Joy': 2,
  ...
  for p,c in zip(pred, correct):
    cm[sen[p]][sen[c]] += 1
  for sentiment in list(sen.keys()):
    index = sen[sentiment]
    tp = cm[index][index]
    fp = 0
    fn = 0
...
def compute_f1(predictions, correct, idx2Label, correctBIOErrors='No', encodingScheme='BIO'): 
    ...
    for sentence in predictions:
        label_pred.extend([idx2Label[element][2:] if len(idx2Label[element]) > 2 else idx2Label[element] for element in sentence])
        
    label_correct = []    
    for sentence in correct:
        label_correct.extend([idx2Label[element][2:] if len(idx2Label[element]) > 2 else idx2Label[element] for element in sentence])
    ...
    le = LabelEncoder()
    labels = ['Annoyance', 'Awe/Sublime', 'Beauty/Joy', 'Humor', 'Nostalgia', 'Sadness', 'Suspense', 'Uneasiness', 'Vitality', 'O']
    le.fit(labels)

    label_correct = [re.sub('[-]', '', label) for label in label_correct]
    label_pred = [re.sub('[-]', '', label) for label in label_pred]

    correct = le.transform(label_correct)
    pred = le.transform(label_pred)
    prec = precision_score(correct, pred, average = "macro")
    rec = recall_score(correct, pred, average = "macro")

    # print(confusion_matrix(correct.argmax(axis=1), pred.argmax(axis=1)))
    # print(f1_score(correct, pred, average = None))
    print(sen_confusion_matrix(label_pred, label_correct))
    
    f1 = f1_score(correct, pred, average='macro')
    # if (rec+prec) > 0:
    #     f1 = 2.0 * prec * rec / (prec + rec);
